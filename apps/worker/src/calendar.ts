...
const BATCH_WAIT_MS = 300_000; // Wait 5 min after last photo for batch to "close"
...
export async function addPhotoToBatch(
    ...
    if (photoBatches.has(key)) {
        ...
        if (count <= 5) {
            await sendTelegramMessage(chatId, `📷 Foto ${count} recebida! Envie mais ou aguarde 5 min para criar o calendário.`);
        } else if (count === 28) {
        ...
        await sendTelegramMessage(chatId,
            ...
            `Envie todas as fotos que quiser nos próximos 5 minutos.\n` +
